...
class DC_OPF(ProblemClass):
    ...
    def __init__(self, network):
        ...
        self.num_constraints = 2 * self.n_gens # consider generator limits only
    ...
    def generate_data(self, num_scenarios, data_file, seed=42):
        """
    ...
    def render(self, inputs, outputs, center, norm_type, norm_value):
        ...
        points_dict = {norm_type: inputs}
        plot_3d_norm_balls(center, points_dict=points_dict, norm_types=(norm_type,), radius_dict=radius_dict, same_figure=True)


    def compute_new_center(self, B, nn_model = None):
        """
        Compute the new center based on the bad points and the neural network model.

        Args:
            B (list): List of bad points.
            nn_model (torch.nn.Module): The neural network model.

        Returns:
            np.array: The new center.
            int: The number of points used to compute the new center.
        """

        if nn_model is None:
            print(f"A neural network model is required to compute the new center.")
            return new_center, len_mean

        new_center = np.mean(B, axis=0)
        len_mean = len(B)   
        output = nn_model(torch.tensor(new_center).float())
        list_copies = [deepcopy(B)]
        while not all(self.gen_limits[i][0] <= output[i] <= self.gen_limits[i][1] for i in range(self.output_size)):
            for element in list_copies:
                combinations = list(itertools.combinations(element, len(element) - 1))
                for combo in combinations:
                    new_center = np.mean(combo, axis=0)
                    len_mean = len(combo)
                    output = nn_model(torch.tensor(new_center).float())
                    if all(self.gen_limits[i][0] <= output[i] <= self.gen_limits[i][1] for i in range(self.output_size)):
                        return new_center, len_mean
        return new_center, len_mean
